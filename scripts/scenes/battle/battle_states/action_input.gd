...
func _print_special_prompt(cur_unit: UnitRun) -> void:
	...
	Console.print_line("* Currently selecting [%s]" % cur_unit.specials[selected_special_idx])
		...
		if choosing_attack_or_special:
			...
			if data.input_data.inputs[BattleInputData.InputType.LEFT] or data.input_data.inputs[BattleInputData.InputType.RIGHT]:
				...
				if attack_selected:
					...
					_print_special_prompt(cur_unit)
				else:
			...
			if data.input_data.inputs[BattleInputData.InputType.BACK]:
				...
				data.input_data.clear_inputs()
				selected_special_idx -= 1
				if selected_special_idx < 0:
					selected_special_idx = cur_unit.specials.size() - 1
				Console.print_line("* Currently selecting [%s]" % cur_unit.specials[selected_special_idx])
			
			elif data.input_data.inputs[BattleInputData.InputType.RIGHT]:
				data.input_data.clear_inputs()
				selected_special_idx += 1
				if selected_special_idx > cur_unit.specials.size() - 1:
					selected_special_idx = 0
				Console.print_line("* Currently selecting [%s]" % cur_unit.specials[selected_special_idx])
			
			elif data.input_data.inputs[BattleInputData.InputType.CONFIRM]:
				data.input_data.clear_inputs()
				if cur_unit.mp >= cur_unit.specials[selected_special_idx].mp_cost:
					state = special
				else:
					Console.print_line("! [%s] insufficient MP (%d / %d)" % [cur_unit, cur_unit.mp, cur_unit.specials[selected_special_idx].mp_cost], Color.RED)
